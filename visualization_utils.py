...
send = socket_sender('127.0.0.1',20000)
########################################
...
####################################################################################
  ...
  for display_str in display_str_list[::-1]:
   #send.send_data(display_str)
    print display_str
    if display_str[0:13] == 'traffic light':
      if int(display_str[15:17]) > 50:
        flag1=1
        box_image = (int(left), int(top), int(right), int(bottom))
        region = image.crop(box_image)
	...
	if r > g:
	    print "red"
            sender.send_data('stop')
            send.send_data('前方发现红灯')
	else:
	    print "green"
            sender.send_data('start')
            send.send_data('前方为绿灯')
        ...
        #th = cv2.inRange(hue_image, low_range, high_range)	
          ...
          #sender.send_data('start')
        #else:
          #print "red lights"########################START
          #sender.send_data('stop')
          #sender.send_data('stop')																																														')
	      #else: print 'green lights'
        region.show()
        #index1 = time.time()
        #region.save('traffic'+str(index1)+'.png')
  if flag1 == 0: print 'no traffic lights'
######################################################################################
  '''draw.line([(left, top), (left, bottom), (right, bottom),																																																																																																																																							
             (right, top), (left, top)], width=thickness, fill=color)'''
########################################################################################
  #image1 = cv2.imread('/home/lixuan/Desktop/traffic_lights/test_images/image3.jpg')
  '''box_image = (left, top, right, bottom)
  region = image.crop(box_image)
  region.show()
  index1 = time.time()
  region.save('hello'+str(index1)+'.png')'''
#######################################################################################
  try:
